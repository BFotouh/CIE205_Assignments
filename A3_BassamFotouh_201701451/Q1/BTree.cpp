...
//////////////////////         Q 1          ////////////////////////////////////
bool BTree::rec_CheckMinBiHeap(Node* subRoot)
{
	if (subRoot != nullptr)
	{
		if (subRoot->getdata() <= subRoot->getleft()->getdata()
			&& subRoot->getdata() <= subRoot->getright()->getdata()
			&& rec_CheckMinBiHeap(subRoot->getleft()) && rec_CheckMinBiHeap(subRoot->getright()))
			return true;
	}
	else
		return true;
...
bool BTree::CheckMinBiHeap() const
{
	return rec_CheckMinBiHeap(root);
}
